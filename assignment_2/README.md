...
Implement all the necessary files and then build the assignment_2 package using
~~~~bash
colcon build --symlink-install --packages-select assignment_2
~~~~
...
~~~~bash
ros2 run assignment_2 min_jerk
~~~~
...
~~~~bash
ros2 run assignment_2 move_joint --1b
~~~~
...
~~~~bash
ros2 run assignment_2 move_joint --1c
~~~~
...
~~~~bash
ros2 run assignment_2 move_joint --1d
~~~~
...
Run A* on 2D space
~~~~bash
cd src/cs477_ws/assignment_2/assignment_2
...
Terminal 2: add objects in GAZEBO
~~~~bash
ros2 run assignment_2 add_object
~~~~

Terminal 3: enable the UR5e to avoid obstacles
~~~~bash
ros2 run assignment_2 move_astar
~~~~
